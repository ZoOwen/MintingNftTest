...
const connect = async () => {
  try {
    if (window.ethereum) {
      ...
      alert(
        "Your browser doesn't support ethereum 😔, Install the MetaMask extension"
      );
      return {
        status: false,
        error:
...
const getBlochainData = async (abi) => {
  try {
    ...
    if (networkInfo) {
      const payload = await new window.web3.eth.Contract(
        abi.default.abi,
        networkInfo.address
      );
      return payload;
      ...
      return {
        status: false,
        error: "Could not get network ID from ABI",
      };
  } catch (error) {
